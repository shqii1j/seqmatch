...
def train(args, model, train_loader, val_loader, plotter=None, logger=None, record=False):
    if record:
    ...
    # print(get_time())
    if record:
        losses_record = torch.zeros((len(val_loader.dataset), record))
    for epoch in range(cur_epoch + 1, args.epochs + 1):
        result = train_epoch(args,
                              train_loader,
                              model,
                              criterion,
                              optimizer,
                              ...
                              mixup=args.mixup)
        if record:
            acc1_tr, _, loss_tr, _ = result
            k = 1
        else:
            acc1_tr, _, loss_tr = result
            k = 0

        if epoch % args.epoch_print_freq == k:
            result = validate(args, val_loader, model, criterion, epoch, logger)
            if record:
                acc1, acc5, loss_val, loss_record = result
                losses_record[:, epoch//args.epoch_print_freq] = loss_record
            else:
                acc1, acc5, loss_val = result
        ...
        scheduler.step()
    if record:
        return best_acc1, acc1, losses_record
...
def train_epoch(args,
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if criterion.reduction == "none":
            loss_record = copy.deepcopy(loss.detach().cpu())
    ...
    if criterion.reduction == "none":
        return top1.avg, top5.avg, losses.avg, loss_record
    else:
...
def validate(args, val_loader, model, criterion, epoch, logger=None):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        if criterion.reduction == "none":
            loss_record.append(copy.deepcopy(loss.detach().cpu()))
            loss = torch.mean(loss)
    ...
    if criterion.reduction == "none":
        loss_record = torch.cat(loss_record, dim=0)
        return top1.avg, top5.avg, losses.avg, loss_record
    else:
        return top1.avg, top5.avg, losses.avg
